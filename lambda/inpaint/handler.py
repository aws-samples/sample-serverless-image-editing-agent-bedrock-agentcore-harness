"""Inpaint Lambda function for serverless agentic image editor.

Retrieves a source image and mask from Amazon S3, invokes the Stability AI inpaint
model via Amazon Bedrock, stores the result back in S3, and returns a
...
# Stability AI inpaint model ID
INPAINT_MODEL_ID = "us.stability.stable-image-inpaint-v1:0"
...
def handler(event, context):
    """Lambda entry point for inpainting operations."""
    ...
    try:
        ...
        if not mask_key or not isinstance(mask_key, str):
            duration_ms = int((time.time() - start_time) * 1000)
            _log(request_id, image_keys, duration_ms, "VALIDATION_ERROR")
            return _error_response(
                "VALIDATION_ERROR",
                "mask_key is required and must be a non-empty string",
                request_id,
            )

        # Validate S3 key patterns to prevent path traversal
        import re
        ...
        if not _KEY_PATTERN.match(mask_key):
            duration_ms = int((time.time() - start_time) * 1000)
            _log(request_id, image_keys, duration_ms, "VALIDATION_ERROR")
            return _error_response("VALIDATION_ERROR", "Invalid mask_key format", request_id)
        ...
        image_keys = [source_image_key, mask_key]
        ...
        try:
            mask_response = s3_client.get_object(
                Bucket=IMAGE_BUCKET_NAME, Key=mask_key
            )
            ...
            return _error_response(
                "STORAGE_ERROR",
                f"Failed to retrieve mask image: {str(e)}",
                request_id,
            )
        ...
        model_seed = response_body.get("seed", response_body.get("seeds", [None])[0] if "seeds" in response_body else None)
        ...
        result = {
            ...
            "message": "Successfully inpainted the masked region",
        }
